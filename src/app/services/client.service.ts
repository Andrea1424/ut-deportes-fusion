...
export class ClientService {
  ...
  getPublicacionesId(id: any) {
    return this.http.get(
      `${this.URL}publicaciones/getOneAct.php?idActividad=${id}`
    );
  }
  ...
  deleteReaccion(reaccion: any, publicacion: any) {
    return this.http.get(
      `${this.URL}reacciones/delete.php?idUsuario=${reaccion}&idPublicacion=${publicacion}`
    );
  }

  getIdUsuario(matricula: any) {
    return this.http.get(
      `${this.URL}estudiantes/get.php?matricula=${matricula}`
    );
  }
}
